...
Console console;    // draws the console to the screen (In Progress)
boolean showConsole;
*/
...
class updateAudio implements AudioListener {
  ...
  // default to Right (real) channel if mono
...
void draw()
{
  ...
  background(0);
  //console.draw(0,height-100,width,height,24,20,4,4,color(0,0,0),color(0),color(255));
...
void loadTrack(int _tr) {
  ...
  if ((meta == null) || (player == null) || (tr != track)) {
    ...
    B = player.bufferSize();
    setNSB( B, player.sampleRate(), B );  // bugs when trying to play files @ 32kHz
    if (render == null ) {
...
void mouseWheel(MouseEvent event) {
  ...
  println("zoom: " + round(zoom*100)/100.0);
}
