...
class renderAudio{
  ...
  void drawPPSWave(float[] xIn, float[] yIn, float sc, color c, float sw) {
    ...
    else if (yIn == null && xIn != null) { 
      imax = xIn.length; 
      yIn = new float[imax];
    }
    if (imax > 0 && xIn != null && yIn != null) {
      imax = min(imax-tau*2, N-tau*2);
  ...
  void drawPPSData(float[] LData, float[] RData) {
    ...
    // Right channel data in Red on Real axis
    if (RData != null) {
      drawPPSWave(RData, null, pScale*2, PRed, 1.5);
    }
    // Left channel data in Yellow on Imag axis
    if (LData != null) {
      drawPPSWave(null, LData, pScale*2, PYellow, 1.5);
    }
    // Sum of both channels in White
    if (LData != null && RData != null) {
      drawPPSWave(RData, LData, pScale, PWhite, 1.5);
    }
  }
